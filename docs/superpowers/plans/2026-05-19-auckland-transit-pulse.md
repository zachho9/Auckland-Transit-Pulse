...
- [ ] **Step 8: Create amplify.yml at repo root**
...
- [ ] **Step 9: Verify the frontend builds locally**
...
git commit -m "feat: frontend scaffold with Vite, Tailwind dark theme, and SnapshotContext"
```
...
```bash
cd frontend
VITE_API_URL=https://placeholder.execute-api.ap-southeast-2.amazonaws.com/prod npm run dev
```

Open `http://localhost:5173`. Expect: dark background, "Auckland Transit Pulse" header, "Last updated: Loading…" text, no errors in console.
...
- Map fills the left 62% (dark CartoDB tiles, centred on Auckland)
- Right sidebar shows Scorecard / Worst Routes / Alerts sections
- All panels show "Loading…" text (no data yet — expected)
- No console errors
